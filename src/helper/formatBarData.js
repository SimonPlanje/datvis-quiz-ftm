  ...
  quiz.map((d, index) => {
    if (d.type === 'gokken') {
      ...
      const orderData = rawData
        .slice()
        .sort((a, b) => d3.descending(a.midden, b.midden));
      setTotData(orderData);

      dynamicAnsMoney.push([
        { partij: orderData[0].partij, check: true, id: orderData[0].partij },
        { partij: orderData[1].partij, check: false, id: orderData[1].partij },
        { partij: orderData[2].partij, check: false, id: orderData[2].partij },
      ]);
      ...
      const sortData = groupAge
        .slice()
        .sort((a, b) => d3.descending(a.percentageTotaal, b.percentageTotaal));
      ...
      function removeComma(data) {
        return data.map(
          (d) => (d.percentageTotaal = d.percentageTotaal.replace(',', '.'))
        );
      }
      ...
      dynamicAns.push([
        { partij: cleanData[0].partij, check: true, id: cleanData[0].partij },
        { partij: cleanData[1].partij, check: false, id: cleanData[1].partij },
        { partij: cleanData[2].partij, check: false, id: cleanData[2].partij },
      ]);
    }
  });
}
